...
void runKernelAndMeasureTime(KernelFunc kernel, Node* d_nodes, Node* h_nodes, bool* d_is_completed, unsigned int* d_activeThreadsCount, dim3 blocks, int threadsPerBlock, const char* kernelName) {
    ...
    kernel<<<blocks, threadsPerBlock>>>(d_nodes, d_is_completed, d_activeThreadsCount);
...
int model_main() {
    ...
    for (int i = 0; i < 1; i++) {
        runKernelAndMeasureTime(calculateFunctions, d_nodes, h_nodes, d_is_completed, d_activeThreadsCount, blocks, threadsPerBlock, "calculateFunctions");
